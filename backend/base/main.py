...
from vector_store import create_db, store_report,retrive_data
...
async def student_data(query: str, data: str,Student_Id:int) -> str:
    ...
    student_old_report=retrive_data(Student_Id)
...
async def retriever(query: str) -> list:
    ...
    vectorstore = Chroma(
                  ...
                  persist_directory="Neet_vector_store")
...
async def chat():
    ...
    - Prompts the user to input a unique Student ID.
    - Accepts queries from the user in a loop until 'exit' is typed.
    - Retrieves student-specific data on the first query if available, and generates a response using that data.
    - For subsequent queries, responses are generated based on context retrieval without student-specific data.
    - Stores student reports after the first query.

    Workflow:
        1. Asks for the student's unique Student ID (once per session).
        2. Accepts the user's query or exits on the command "exit".
        3. If the bot is not initialized, it handles student-specific data and stores the response.
        4. For subsequent queries, it generates responses using the context-based RAG method.
        5. Outputs the generated AI response to the user.
    ...
    Notes:
        - The chatbot runs indefinitely until the user types 'exit'.
        - It uses a conversation history stored in memory for managing the conversation context.
        - The `create_db()` function ensures persistent storage of student reports.
    """
    print("Welcome to the NEET Exam Analysis Chatbot!")
    global initialized

    create_db()  # Ensure the database is created (persistent)
    ...
    while True:    
        ...
        if not initialized:
            ...
            store_report(Student_Id, response) # Store the generated response
